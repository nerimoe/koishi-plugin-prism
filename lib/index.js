        ...
        if (cappedWindows.length > 0 || appliedCapAdjustments.length > 0) {
            ...
            if (cappedWindows.length > 0) {
                for (const window of cappedWindows) {
                    ...
                    if (paidBefore > 0)
                        details.push(`之前已计 ${formatNumber(paidBefore)}${currency}`);
                    lines.push(`- ${label}：本次参与 ${formatNumber(currentAmount)}${currency} → 计入 ${formatNumber(amountApplied)}${currency}（${details.join("，")}）`);
                }
            }
            else {
