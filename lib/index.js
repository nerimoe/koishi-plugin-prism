...
exports.Config = koishi_1.Schema.object({
    ...
    powerCommandsAdminOnly: koishi_1.Schema.boolean().default(false).description("关机是否仅允许管理员使用；开机始终仅已入场玩家可用"),
...
class PrismBotClientError extends Error {
    ...
    body;
    originalError;
    constructor(message, code, status, body, originalError) {
        super(message);
        this.code = code;
        this.status = status;
        this.body = body;
...
function applyPrismKoishiPlugin(ctx, config) {
    ...
    ctx.command("coin <deviceRef> [count]", "请求向指定设备投币").action(wrap(async (context, deviceRef, count) => service.coin(await service.sender(context), deviceRef, count)));
    ctx.command("scan <deviceRef>", "使用当前玩家绑定的 Aime 卡刷卡").action(wrap(async (context, deviceRef) => service.scan(await service.sender(context), deviceRef)));
...
class PrismApiClient {
    ...
    async request(method, path, options) {
        ...
        try {
            ...
            if (error?.response) {
                ...
                throw new PrismBotClientError(typeof err.message === "string" ? err.message : `Backend request failed with HTTP ${status}.`, typeof err.code === "string" ? err.code : `HTTP_${status}`, status, body, error);
            }
    ...
    async requestDeviceCommandByIdentity(identity, command, options) {
...
class PrismKoishiService {
    ...
    async coin(sender, rawDeviceRef, rawCount) {
        const deviceRef = cleanText(rawDeviceRef);
        if (!deviceRef)
    ...
    async scan(sender, rawDeviceRef) {
        const deviceRef = cleanText(rawDeviceRef);
        if (!deviceRef)
    ...
    /* ---------------------------- helpers ---------------------------------- */
    async power(sender, deviceRef, state, systemOverride = false) {
    ...
    logDeviceCommandFailure(result) {
        const action = result?.action;
...
function deviceActionFailureReason(error) {
    if (!(error instanceof PrismBotClientError))
        return "请稍后再试";
    if (error.code === "DEVICE_COMMAND_REQUIRES_ACTIVE_SESSION")
        return "玩家未入场";
    if (error.code === "DEVICE_NOT_FOUND")
        return "设备不存在";
    if (error.code === "SCAN_IDENTITY_NOT_BOUND_TO_PLAYER")
        return "玩家未绑定 Aime 卡";
    if (error.code === "COIN_COMMAND_COOLDOWN_ACTIVE")
        return "投币冷却中，请稍后再试";
    return "请稍后再试";
}
function deviceCommandFailureReason(result) {
    const action = result?.action;
    if (!action || (action.status !== "expired" && action.status !== "rejected"))
        return null;
    const message = cleanText(action.payload?.executorFailure?.message);
    if (message.includes("没有在线客户端"))
        return "设备未连接";
    if (message.includes("设备配置不存在"))
        return "设备未配置";
    return "请稍后再试";
}
function deviceCommandLabel(result) {
    return cleanText(result?.action?.payload?.deviceLabel);
}
function genericCommandFailure() {
    return "操作失败，请稍后再试。";
}
